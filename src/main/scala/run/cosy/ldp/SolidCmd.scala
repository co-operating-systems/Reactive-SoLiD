...
object SolidCmd:
   ...
   /** A recursively defined DataSet where the first layer points to a Cofree graph with no metadata
     * and the included sets contain metadata.
     *
     * This differs from RDF1.1 DataSets in that
     *   - DataSets can contain DataSets (which is
     *     [[https://lists.w3.org/Archives/Public/public-n3-dev/2021May/0012.html possible in N3]])
     ...
     *
     * Example: <pre> GraF(ttl"&lt;&gt; a TeaPot.", Set(Cofree( Meta(Uri("/People/.acl"),
     * GraF(ttl"&lt;&gt; a Kettle.", Set()) )) ) </pre>
     */
   type RDataSet = GraF[Cofree[GraF, Meta]]

   /** MetaData on a Request for a DataSet, such as <pre> Cofree( Meta(Uri("/.acl"),
     * GraF(ttl"&lt;&gt; a TeaPot.", Set( ... other datasets ...)) )) </pre> Actually this is badly
     * named. What RDF understands as a DataSet is closer to the type returned by tailForced namely
     * the above defined type RDataSet = GraF[Cofree[GraF, Meta]]
     */
   ...
   /** Build a script that will start by fetching `url`, unmarshall the result into an `Rdf#Graph`
     * inside a Response and apply the continuation `k` to that Response.
     *
     * @param url
     *   to GET
     * @param k
     *   the contination to use once the Response is available
     * @tparam A
     * @return
   ...
   def plainScript(req: HttpRequest): Script[HttpResponse] =
     liftF[SolidCmd, HttpResponse](Plain(req, identity))
   ...
   def plain(req: HttpRequest): SolidCmd[Script[HttpResponse]] =
     Plain(req, Free.pure[SolidCmd, HttpResponse])

   /** Build a Script to fetch owl:imports links starting from a graph at u, avoiding visted ones.
     * See [[https://github.com/solid/authorization-panel/issues/210 Issue 210 on :imports]], though
     * here we use owl.imports provisionally until a better name is chosen.
     *
     * The script returns an ReqDataSet as defined here. It is quite possible that different
     * branches could fetch the same graphs, but it should not lead to infinite loops. todo: there
     * is parallelism here when fetching children: would it work to use Applicatives? todo:
     * returning a Set may be better, but I had trouble getting Traverse to work on Set
     */
   def fetchWithImports(u: Uri, visited: Set[Uri] = Set()): Script[ReqDataSet] =
     for
        response <- get(u)
   ...
   /** Get request from URL, but the response should be interpreted to a manipulatable Scala object.
     * Eg. an RDF Graph for an RDF resource.
     *
     ...
     *   - content-type?
     *   - ... It captures a request that reutrns a Response, where k is a function to transform
     *     that response into any A
     */
   case class Get[A](url: Uri, k: Response => A) extends SolidCmd[A]

   /** This is the base class of commands that covers all the Http Requests. This is essentially
     * done in terms of Execution of such a Request will return an HttpResponse
     *
     * @param k
   ...
   /** some Metadata on the resource from the server.
     *
     * @param the
     *   response on the message - whether the request failed or not
     * @param url
     *   the URL of the resource
     * @param headers
     *   : Seq of Http Headers (this could also be represented as a Graph)
     */
   ...
   /** A response to a request. The Metadata tells if the response succeeded, what the problems may
     * have been, etc... The content, is an attempted parse of the stream to the object type.
     * (todo?: generalise the type of the content to quite a lot more types, such as DataSets,
     * Images, streams, ...)
     */
   ...
   given GraFTraverse: cats.Traverse[GraF] with
      ...
      override def foldLeft[A, B](fa: GraF[A], b: B)(f: (B, A) => B): B = fa.other.foldLeft(b)(f)
