...
/** The Coroner can be used to print a diagnostic report of the JVM state, including stack traces
  * and deadlocks. A report can be printed directly, by calling `printReport`. Alternatively, the
  * Coroner can be asked to `watch` the JVM and generate a report at a later time - unless the
  * Coroner is cancelled by that time.
  *
  * The latter method is useful for printing diagnostics in the event that, for example, a unit test
  * stalls and fails to cancel the Coroner in time. The Coroner will assume that the test has "died"
  * and print a report to aid in debugging.
  */
object Coroner:

   /** Used to cancel the Coroner after calling `watch`. The result of this Awaitable will be `true`
   ...
   /** Ask the Coroner to print a report if it is not cancelled by the given deadline. The returned
     * handle can be used to perform the cancellation.
     *
   ...
   def watch(
      ...
      def triggerReportIfOverdue(duration: Duration): Unit =
         ...
         try
            if !Await.result(watchedHandle, duration) then
   ...
   def printReport(reportTitle: String, out: PrintStream): Unit =
      ...
      def findDeadlockedThreads: (Seq[ThreadInfo], String) =
         val (ids, desc) =
           if threadMx.isSynchronizerUsageSupported() then
      ...
      def printThreadInfos(threadInfos: Seq[ThreadInfo]) =
        if threadInfos.isEmpty then
           println("None")
        else
           for ti <- threadInfos.sortBy(_.getThreadName) do println(threadInfoToString(ti))
      ...
      def threadInfoToString(ti: ThreadInfo): String =
         ...
         if ti.getLockName != null then
            sb.append(" on " + ti.getLockName)

         if ti.getLockOwnerName != null then
            sb.append(" owned by \"")
            sb.append(ti.getLockOwnerName)
            sb.append("\" Id=")
            sb.append(ti.getLockOwnerId)

         if ti.isSuspended then
            sb.append(" (suspended)")

         if ti.isInNative then
            sb.append(" (in native)")
         ...
         for i <- 0 until stackTrace.length do
...
/** Mixin for tests that should be watched by the Coroner. The `startCoroner` and `stopCoroner`
  * methods should be called before and after the test runs. The Coroner will display its report if
  * the test takes longer than the (dilated) `expectedTestDuration` to run.
  *
  * If displayThreadCounts is set to true, then the Coroner will print thread counts during start
  * and stop.
  */
