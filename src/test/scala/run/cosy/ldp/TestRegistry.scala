...
class TestRegistry extends munit.FunSuite:
   ...
   test("On ATree[Int, Option[String]]") {
     ...
     assertEquals(rt4, DirTree(1, None, HashMap("src" ->
       DirTree(2, Some("coucou"), HashMap("test" ->
         DirTree(3, None)))))
     )

     val rt4new = rt4.replace(Some("55"), List("src", "test"))
     assertEquals(rt4new, DirTree(1, None, HashMap("src" ->
       DirTree(2, Some("coucou"), HashMap("test" ->
         DirTree(3, Some("55"))))))
     )
   ...
   test("on path /") {
     ...
     dbInt.removePath(p2)
     assertEquals(dbInt.getActorRef(p3), Some((List("src", "test"), 1, Some(1) )), dbInt.pathMap.get())
   ...
   test("Boolean Attr") {
     ...
     assertEquals(rt3, DirTree(1, true, HashMap("src" ->
       DirTree(2, false, HashMap("test" ->
         DirTree(3, true)))))
     )
     ...
     assertEquals(rt3.findClosestRs(srcTstScHello){ identity }, (srcTstScHello.drop(2), 3, Some(3)))
   }
...
//     // test a few values in dts
     ...
     val dt2 = dts(2)
     assertEquals(dt2, DirTree(0, true, HashMap("1" ->
